...
const MedicineEdit = () => {
  ...
  const inputMedicineName = useCallback(
    (event) => {
  ...
  const inputIngestionTimesPerDay = useCallback(
    (event) => {
  ...
  const inputIngestionAmountPerDay = useCallback(
    (event) => {
  ...
  const unitOptions = [
    ...
    <MenuItem key="錠" value="錠">
      錠
    </MenuItem>,
    <MenuItem key="枚" value="枚">
      枚
    </MenuItem>,
    ...
    <Container maxWidth="sm">
      <Box sx={{ mt: 4 }}>
        ...
        <TextInput
          ...
          multiline={false}
          required={true}
          onChange={inputIngestionTimesPerDay}
          rows={1}
          value={ingestionTimesPerDay}
          type={'number'}
          sx={{ mt: 2 }}
        />
        <TextInput
          ...
          multiline={false}
          required={true}
          onChange={inputIngestionAmountPerDay}
          rows={1}
          value={ingestionAmountEveryTime}
          type={'number'}
          sx={{ mt: 2 }}
        />
        <FormControl fullWidth required sx={{ mt: 2 }}>
          <InputLabel id="unit-label">単位</InputLabel>
          <Select labelId="unit-label" id="unit" value={unit} label="単位" onChange={handleUnitChange}>
            {unitOptions}
          </Select>
        </FormControl>
        <Box sx={{ mt: 3, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <PrimaryButton label={'薬を更新'} onClick={handleUpdateMedicine} sx={{ mb: 1 }} />
